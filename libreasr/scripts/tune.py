...
def tune_everything(config, factory_fn, dataset, fraction=1.0, chdir="", do_print=False, sanity_check=False):
    ...
    if sanity_check:
        ...
        # test lm generation
        res = li.model.lm.generate("the other day when we visited san", lang=li.model.lang, steps=64)
        print("[sanity-check] [LM] Generation :", res)
        return
    
    # helper
    def fx(a):
        return int((a / 1000.) * 16000)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_id", type=str, help="Which model_id to load."
    )
    parser.add_argument(
        "--dataset",
        type=str,
