...
def process_one(f, get_labels, allow_no_label=False):
    rows = []
    try:
        if f.suffix == ".m4a":
            raise Exception(".m4a isn't an audio file: " + str(f))
        aud, sr = torchaudio.load(f)
        assert aud.size(0) >= 1 and aud.size(1) >= 1
        xlen = int((aud.size(1) / float(sr)) * 1000.0)
        labels = get_labels(f, duration=xlen)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dataset",
        type=str,
        help="which dataset: common-voice | tatoeba | tf-speech | voxlingua107 | librispeech | yt",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="no",
        type=str,
        help="save audio files that don't contain a label (yes | no)",
    )
    parser.add_argument(
        "--out",
    ...
    allow_no_label = args.allow_no_label == "yes"
    ...
    # filter out bad ones
    print("> Removing bad rows (bad=True)...")
    df = df[df.bad == False]
