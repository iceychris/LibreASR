...
CHANGES:
Changed to the reference code
(https://github.com/speechbrain/speechbrain/blob/develop/speechbrain/decoders/transducer.py):
 * remove language model stuff
 * adapt to different transducer model inputs and outputs
"""
...
class TransducerBeamSearcher(torch.nn.Module):
    ...
    def forward_init(self, bs=1, device="cpu"):
        ...
        for i_batch in range(bs):
            ...
            blank = torch.ones((1, 1), device=device, dtype=DTYPE) * self.blank_id
            input_PN = torch.ones((1, 1), device=device, dtype=DTYPE) * self.bos_id
        ...
        self.s_blank = blank
        self.s_input_PN = input_PN
        self.s_hyp = hyp
        self.s_beam_hyps = beam_hyps

    def forward_step(self, tn_output):
        # get state
        blank = self.s_blank
        input_PN = self.s_input_PN
        hyp = self.s_hyp
        beam_hyps = self.s_beam_hyps
        i_batch = 0
        nbest_batch = []
        nbest_batch_score = []
    ...
    def transducer_beam_search_decode(self, tn_output):
        ...
        for i_batch in range(tn_output.size(0)):
            ...
            # Prepare Blank prediction
            blank = (
                torch.ones((1, 1), device=tn_output.device, dtype=DTYPE) * self.blank_id
            )
            input_PN = (
                torch.ones((1, 1), device=tn_output.device, dtype=DTYPE) * self.blank_id
            )
