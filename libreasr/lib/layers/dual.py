...
class DualModeSelfAttention(nn.Module):
    def __init__(self, dim, inner_dim, **kwargs):
        ...
        self.attn_stream = LocalAttention(
            causal=True, look_backward=1, look_forward=0, **kwargs
        )
        self.attn_full = LocalAttention(
            causal=False, look_backward=1, look_forward=1, **kwargs
        )
...
class DualModeMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, stream=True):
        ...
        q, k, v = map(
            lambda x: x[0](x[1]), zip((self.to_q, self.to_k, self.to_v), (x,) * 3)
        )
